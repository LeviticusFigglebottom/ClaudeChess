...
Phase 1's bot-calibration gate (±75 Elo, ≥200 games/band — the gate not to skip) closes in the dedicated calibration sessions; until it does, bots log `bot running UNCALIBRATED params` (theirs, not a defect). Beyond that: the deployment pass — **fully specified in `docs/DEPLOYMENT.md` and blocked solely on credentials** (a Vercel project, a Supabase project, an `ANTHROPIC_API_KEY`; none exist in the dev container). The runbook carries the exact env matrix, the migration/seed steps, and the five measured gates (isolation + browser gate vs the preview, the live anonymous→verified conversion chain, `explorer.lichess.ovh` from production egress, LLM usage-metering/cache verification, Playwright vs the deployed URL). Last: crazyhouse if a drop-capable board ever justifies it (B1.1).
