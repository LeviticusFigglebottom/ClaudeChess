...
| D — UNCLEAR on the blunder sample (spec bar < 15%, voided post-gate; revised target < 18% with the structural class) | ❌ **24.0%** (43/179; 26.8% before the structural class; 30.1% across all error classes; 19.2% on the pre-correction loss≥30 class). Target not reached — reported per instruction, see below |
...
| **(b) Live Supabase auth round-trip** | ✅ 14/15 (final leg pending a dashboard toggle) | Anonymous sign-in provisions users rows; prefs write; challenge-link → live game → move → resign → **archive into history**; conversion lands on the **same `users.id`** (A2.1 links-never-copies verified against the live service); prefs + history survive; ratings snapshot unchanged (trivially — unrated game, no rating rows). Pending: the UI confirmation leg — GoTrue's **"Secure email change" (default ON) rejects anonymous conversions** (`Email address "" is invalid`); needs the toggle off. Confirmation was completed via the admin API for the persistence checks; the emailed-link leg itself remains unexercised (no mailbox; `generate_link` 404s on this GoTrue) |
...
| **(e) Two-context multiplayer over the real network** | ✅ 5/6 — drift bound does not hold remotely | 60+0 game via challenge flow; **DOM-to-DOM propagation median 1107ms / p95 1873ms** (server-visibility median **312ms** — the rest is client read cadence; Realtime websockets connect but delivery was poll-dominated); real flagfall finalizes server-side (`time forfeit`, result on both clients, zero page errors); **drift `remainingRawMs` = −399ms / −424ms across two real flags vs the 200ms bound** — on this deployment drift ≈ the latency of the first state read past zero (floor ≈ the ~312ms round-trip), so the in-container −41ms bound is a loopback artifact; the server-side guarantee (never finalize early, signed evidence recorded) holds |

Deployed findings fixed during the pass (each pinned by test or driver): Vercel Hobby cron limits reject >2 or sub-daily crons at config validation → consolidated `/api/cron/daily` dispatcher + **lazy correspondence finalization on state read** (`live-daily.test.ts`); `NEXT_PUBLIC_SUPABASE_URL` pasted in dashboard REST form breaks every auth call → normalized in `src/lib/supabase/env.ts`; **GoTrue anonymous users carry `email: ""` and collided on `users_email_unique`** — every anonymous visitor after the first failed to provision (PGlite fixtures used `null`; only the live service surfaced `""`) → normalized at adapter + domain, regression-pinned; `LlmUnavailableError` mapped to 503.
