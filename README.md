...
## Background analysis, a portal fix, and presets — the onboarding round

**Analysis and import now survive tab changes.** They used to run inside page components: navigating away unmounted the page, orphaned the engine worker, and threw away every progress indicator (a revisit even booted a duplicate engine). Both now live in a module-level singleton (`src/lib/analysis/runner.ts`) that pages merely enqueue into and subscribe to; a header chip shows live progress on every page with a stop button, review pages refresh as passes land, and the dashboard/games cards mirror the same snapshot. Machine-verified: started "Analyze all" on /games, clicked to /puzzles mid-run, and watched the chip advance pass2 3/25 → 21/25 → pass3 3/4 while solving-screen puzzles rendered — 31/33 plies reached depth ≥ 18 with the games page long unmounted (the global eval cache pre-covered 9 book plies of the test game for free). En route this fixed a real gap: bot games have no ply rows until first analysis, and the old `plyCount > 0` filter silently excluded them from Analyze-all forever (`needsAnalysis` now treats a moveless-row local game as "never analyzed").

**The clipped sign-in dialog was a CSS containing-block bug.** The header's `backdrop-blur` makes it the containing block for `position: fixed` descendants, so the auth modal centered itself in the 52px header strip — its top half (the email field) rendered above the viewport. The dialog now portals to `document.body` (plus a `max-h-[92vh]` scroll guard for tiny screens).

**Theme filtering had NEVER worked — found while building presets.** The original puzzle seed `JSON.stringify`'d the themes and moves arrays into jsonb columns, storing jsonb *string scalars*; drizzle's read path accidentally double-parses them back to arrays (which is why puzzles played fine), but SQL saw scalars, so `themes ?| array['fork']` matched **0 of 31,224 rows** — every themed query (including the fingerprint's drill-deck links) silently served unfiltered puzzles. Migration 0020 re-casts the data (`(themes #>> '{}')::jsonb`), the seed now writes through `jsonb_to_recordset`, and a PGlite test pins the filter actually restricting. After the fix: fork 4,043 · mateIn1 3,428 · mateIn2 3,289 · pin 2,199 · endgame 15,705.
...
## Security-advisor criticals — the data API is now closed by construction

Supabase's advisor flagged two criticals on the hosted project (`rls_disabled_in_public`, `sensitive_columns_exposed`), and the probe confirmed them: the **browser-shipped anon key could read every public table through the auto-generated PostgREST API** — `users` (with emails), `games`, `sessions`, `puzzles` all answered 200 with rows, and writes would have been accepted the same way. The app itself never used that surface (every query runs over the direct Postgres connection as the table owner; auth lives in the `auth` schema; the Realtime poke is a broadcast channel that reads no tables), so the whole thing closes with zero behavior change. Migration `0021_lock_public_api_surface` enables **RLS with zero policies on all 28 public tables** (the owner connection bypasses non-FORCE RLS — verified end-to-end locally: 28/28 locked, all API routes still green) and **revokes every grant from `anon`/`authenticated`**, including default privileges so future tables are born closed. The G5 gate (`npm run db:verify`) now fails any table that ships without RLS and any surviving API-role grant, making the lockdown self-enforcing. Post-deploy, the same probe that read rows gets `permission denied for table users` on every table.
