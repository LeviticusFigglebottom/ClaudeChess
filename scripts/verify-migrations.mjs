...
// All 26 tables present (17 through Phase 1.5; +linked_accounts/tb_cache/
// explorer_cache in Phase 2/3; +live_games/live_game_events/matchmaking_queue
// in Phase 4; +tempo_attempts/llm_cache in Phase 5).
await expect("26 tables exist", async () => {
  const result = await db.query(
    `select count(*)::int as n from information_schema.tables where table_schema = 'public'`
  );
  if (result.rows[0].n !== 26) throw new Error(`expected 26 tables, found ${result.rows[0].n}`); // +explorer_agg (0015)
});
